...
module vacuum_equilibrium
  ...
  subroutine equilibrium_VFB(my_id)
   ...
   do i=1, n_pf_coils
     ! Kinetic-RE q-amplitude control: an ADDITIVE shaping current on top of
     ! the base current, applied to every PF coil including those carrying no
     ! position feedback (those are set once at allocation and never revisited
     ! below). It composes with the position feedbacks instead of competing
     ! with them for coils -- a coil may serve only one FEEDBACK channel (see
     ! the abort below), but this is a separate additive term and the
     ! direction is position-neutral by construction, so the loops do not
     ! fight. Guarded on /= 0 so a run without the control is bit-identical.
